...
### Карпейкин Дмитрий
- Роль: TL
- Зона ответственности: разработка WSGI-фреймворка, архитектура приложения(маршрутизация, взаимодействие компонентов фреймворка), совместная разработка демонстрационного веб-приложения
- Реализованные задачи и функциональность:
  - реализован WSGI-фреймворк на Python с поддержкой маршрутизации (@app.route), обработки GET и POST запросов
  - разработан класс App, который отвечает за регистрацию маршрутов, обработку различных ошибок, формирование ответов через класс Response и запуск сервера
  - реализован класс Request для парсинга пути, метода запроса, заголовков, query-параметров и тела запроса, включая поддержку JSON формата
  - реализован класс Response для формирования корректного HTTP-ответа с нужными статусами и заголовками, поддержкой HTML и JSON
  - разработан класс Router, который хранит маршруты и выбирает соответствующий обработчик по пути и методу запроса
  - реализована возможность добавление новых маршрутов
### Пузырников Даниил
- Роль: Testing
- Зона ответственности: покрытие грейворка тестами, проверка корректности маршрутов, обработка запросов и ответов, совместная разработка демонстрационного веб-приложения
- Реализованные задачи и функциональность:
  - test_app: проверяет маршрутизацию, обработку GET и POST-запросов, корректное формирование WSGI-ответа, обработку ошибок и возврат статусных кодов
  - test_request: тестирует разбор пути, метода, заголовков, query-параметров и тела запроса, включая JSON формат и обработку ошибок при некорректном JSON
  - test_response: проверяет формирование статуса, строки статуса, заголовков, Content-Length, кодирование Unicode и работу с кастомными Content-Type.
  - test_router: проверяет регистрацию маршрутов, работу одного пути с несколькими методами, разрешение маршрутов и корректное различение GET и POST
### Совместное
Также совместно был написано веб-приложение, которое показывает работу WSGI-фреймворка: маршрутизацию, обработку GET/POST-запросов, работу с query-параметрами, JSON формат и обработка ошибок.
## Итог
Таким образом мы получили WSGI-фреймворк, который реализует базовую архитектуру веб-приложения: он регистрирует маршруты через декоратор @app.route, обрабатывает HTTP-запросы, превращая их в объект Request, и возвращает ответы в виде объекта Response. Фреймворк поддерживает обработку GET и POST запросов, работу с query-параметрами, а также возврат кодов ошибок. Для демонстрации работы было написано веб-приложение, которое можно запускать на локальном сервере, а зарегистрированные обработчики автоматически вызываются при поступлении запросов.